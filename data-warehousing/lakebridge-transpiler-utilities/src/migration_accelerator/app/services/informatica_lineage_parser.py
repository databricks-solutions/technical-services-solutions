...
Rows whose object basename is a SQL pseudo-table (e.g. Oracle ``DUAL``) are omitted
from mapping-to-object edges so they do not appear in lineage or migration planning.
"""
...
class InformaticaLineageParser:
    ...
    def parse_infa_sheets(sheets: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        embedded_programs_key = sheets_lower.get("embedded sql programs")
        embedded_functions_key = sheets_lower.get("embedded sql functions")
        # Prefer the RAW_ long-form xref (Program/Object/Operation/Count) when
        # available - the named "Embedded SQL Program-Obj Xref" sheet is in
        # wide pivoted format and cannot be parsed by the SQL classifier.
        embedded_xref_key = (
            sheets_lower.get("raw_program_object_xref")
            or sheets_lower.get("embedded sql program-obj xref")
            or sheets_lower.get("embedded sql program-object xref")
        )
        ...
        # ---- Embedded SQL classification (refine TABLE_OR_VIEW -> TABLE/VIEW) -
        # Reuse the SQL object classifier on the analyzer's Embedded SQL sheets,
        # which carry the Script Category signal we need. Only refines nodes
        # that are currently TABLE_OR_VIEW; FLAT_FILE classification from the
        # System Types path is preserved as-is.
        embedded_refined = 0
        if embedded_xref_key:
            try:
                from migration_accelerator.app.services.sql_object_classifier import (
                    SQLObjectClassifier,
                )

                xref_df = sheets[embedded_xref_key]
                programs_df = (
                    sheets[embedded_programs_key] if embedded_programs_key else None
                )
                functions_df = (
                    sheets[embedded_functions_key] if embedded_functions_key else None
                )

                classifier = SQLObjectClassifier(
                    sql_programs_df=programs_df, functions_df=functions_df
                )
                classifier.build_lookups()

                # Walk Embedded SQL Program-Obj Xref to gather creating
                # programs and operations per object (same shape as SQL parser).
                creating_programs_by_obj: Dict[str, set] = {}
                referencing_programs_by_obj: Dict[str, set] = {}
                ops_by_obj: Dict[str, set] = {}
                obj_display: Dict[str, str] = {}

                xref_df = xref_df.copy()
                xref_df.columns = xref_df.columns.astype(str).str.strip()
                xref_program_col = _find_column(
                    xref_df, ["Program", "Script", "Program Name"]
                )
                xref_object_col = _find_column(xref_df, ["Object", "Object Name"])
                xref_op_col = _find_column(xref_df, ["Operation"])

                if xref_program_col and xref_object_col:
                    cols = list(xref_df.columns)
                    p_idx = cols.index(xref_program_col)
                    o_idx = cols.index(xref_object_col)
                    op_idx = cols.index(xref_op_col) if xref_op_col else None

                    for row in xref_df.itertuples(index=False):
                        program = row[p_idx]
                        obj = row[o_idx]
                        ...
                        obj = str(obj).strip()
                        if not program or not obj:
                            continue
                        operation = (
                            str(row[op_idx]).strip().upper()
                            if op_idx is not None and not pd.isna(row[op_idx])
                            else ""
                        )
                        obj_id = obj.lower()
                ...
                # Refine each TABLE_OR_VIEW node whose name matches an embedded-
                # SQL object. Drop CTEs / function false positives surfaced by
                # the classifier as well.
                drop_ids: List[str] = []
                for node_id, node in nodes_dict.items():
                    if node.get("type") != NodeTypeHelper.TABLE_OR_VIEW:
                        continue
                    label = node.get("label") or node.get("name") or node_id
                    obj_id = (label or "").lower()
                    if obj_id not in obj_display:
                        continue
                    new_type, drop_reason = classifier.classify_with_referencing_programs(
                        object_name=label,
                    ...
                    if drop_reason is not None:
                        ...
                        continue
                    if new_type and new_type != NodeTypeHelper.TABLE_OR_VIEW:
                        node["type"] = new_type
                        embedded_refined += 1

                if drop_ids:
                    for d in drop_ids:
                        nodes_dict.pop(d, None)
                    all_edges[:] = [
                        e for e in all_edges
                        if e["source"] not in drop_ids and e["target"] not in drop_ids
                    ]
                    log.info(
                        f"Informatica embedded SQL classifier dropped "
                        f"{len(drop_ids)} false-positive nodes"
                    )
                if embedded_refined:
                    log.info(
                        f"Informatica embedded SQL classifier refined "
                        f"{embedded_refined} TABLE_OR_VIEW nodes into "
                        f"explicit subtypes (TABLE/VIEW/etc.)"
                    )
            except Exception as exc:
                log.warning(
                    f"Embedded SQL classification failed (continuing with legacy "
                    f"types): {type(exc).__name__}: {exc}"
                )
        ...
        log.info(
            ...
            f"(table={counts['table']}, view={counts['view']}, "
            f"external={counts['external']}, table_or_view={counts['table_or_view']}), "
