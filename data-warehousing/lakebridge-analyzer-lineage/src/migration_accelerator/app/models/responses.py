...
class LineageInsightsResponse(BaseModel):
    ...
    total_tables: int = Field(
        ...,
        description=(
            "Total number of persisted-data nodes (TABLE / VIEW / "
            "MATERIALIZED_VIEW / FLAT_FILE plus the "
            "legacy TABLE_OR_VIEW fallback)"
        ),
    )
...
class MigrationOrderResponse(BaseModel):
    ...
    groups: List[Dict[str, Any]] = Field(
        ...,
        description=(
            "Migration groups with ordered waves. "
            "SQL and Informatica graphs may include a final group with group_name "
            "'Independent files' and independent_files_bucket true when multiple "
            "singleton-isolated migration units share no cross-group dependencies."
        ),
    )
