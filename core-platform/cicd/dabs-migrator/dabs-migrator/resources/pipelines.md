...
resources:
  pipelines:
    <pipeline_name>:
      ...
      channel: <string>  # string | SDP Release Channel that specifies which version to use.
      clusters:  # array[object] | Cluster settings for this pipeline deployment.
        -
          ...
          autoscale:  # object | Parameters needed in order to automatically scale clusters up and down based on
            ...
            mode: ENHANCED  # enum: ENHANCED, LEGACY | Databricks Enhanced Autoscaling optimizes cluster utilization by automatically
      ...
      gateway_definition:  # PRIVATE PREVIEW | object | The definition of a gateway pipeline to support change data capture.
        connection_id: <string>  # DEPRECATED | PRIVATE PREVIEW | string | [Deprecated, use connection_name instead] Immutable. The Unity Catalog connectio
        connection_name: <string>  # REQUIRED | PRIVATE PREVIEW | string | Immutable. The Unity Catalog connection that this gateway pipeline uses to commu
        ...
        gateway_storage_catalog: <string>  # REQUIRED | PRIVATE PREVIEW | string | Required, Immutable. The name of the catalog for the gateway pipeline's storage
        gateway_storage_name: <string>  # PRIVATE PREVIEW | string | Optional. The Unity Catalog-compatible name for the gateway storage location.
        gateway_storage_schema: <string>  # REQUIRED | PRIVATE PREVIEW | string | Required, Immutable. The name of the schema for the gateway pipelines's storage
      ...
      ingestion_definition:  # object | The configuration for a managed ingestion pipeline. These settings cannot be use
        ...
        connector_type: CDC  # enum: CDC, QUERY_BASED | [Beta] (Optional) Connector Type for sources. Ex: CDC, Query Based.
        data_staging_options:  # object | [Beta] (Optional) Location of staged data storage. This is required for migratio
          catalog_name: <string>  # REQUIRED | string | [Beta] (Required, Immutable) The name of the catalog for the connector's staging
          schema_name: <string>  # REQUIRED | string | [Beta] (Required, Immutable) The name of the schema for the connector's staging
          volume_name: <string>  # string | [Beta] (Optional) The Unity Catalog-compatible name for the storage location.
        full_refresh_window:  # object | (Optional) A window that specifies a set of time ranges for snapshot queries in
          days_of_week:  # array[string] | Days of week in which the window is allowed to happen
            - <value>
        ...
        ingest_from_uc_foreign_catalog: <bool>  # bool | Immutable. If set to true, the pipeline will ingest tables from the
        ingestion_gateway_id: <string>  # string | Identifier for the gateway that is used by this ingestion pipeline to communicat
        netsuite_jar_path: <string>  # PRIVATE PREVIEW | string | Netsuite only configuration. When the field is set for a netsuite connector,
        ...
        source_configurations:  # array[object] | Top-level source configurations
          -
            ...
            google_ads_config: <...(nested)>  # PRIVATE PREVIEW | ...(nested)
        table_configuration:  # object | Configuration settings to control the ingestion of tables. These settings are ap
          auto_full_refresh_policy:  # object | (Optional, Mutable) Policy for auto full refresh, if enabled pipeline will autom
            enabled: <bool>  # REQUIRED | bool | (Required, Mutable) Whether to enable auto full refresh or not.
            min_interval_hours: <int>  # int | (Optional, Mutable) Specify the minimum interval in hours between the timestamp
          clustering_columns:  # PRIVATE PREVIEW | array[string] | List of column names to use for clustering the destination table.
            - <value>
          enable_auto_clustering: <bool>  # PRIVATE PREVIEW | bool | Whether to enable auto clustering on the destination table.
          ...
          primary_keys:  # array[string] | The primary key of the table used to apply changes.
            - <value>
          query_based_connector_config:  # object | Configurations that are only applicable for query-based ingestion connectors.
            cursor_columns:  # array[...(nested)] | The names of the monotonically increasing columns in the source table that are u
              - <value>
            deletion_condition: <string>  # string | Specifies a SQL WHERE condition that specifies that the source row has been dele
            hard_deletion_sync_min_interval_in_seconds: <int>  # int | [Beta] Specifies the minimum interval (in seconds) between snapshots on primary
          row_filter: <string>  # string | (Optional, Immutable) The row filter condition to be applied to the table.
          ...
          scd_type: SCD_TYPE_1  # enum: SCD_TYPE_1, SCD_TYPE_2, APPEND_ONLY | The SCD type to use to ingest the table.
          sequence_by:  # array[string] | The column names specifying the logical order of events in the source data. Spar
            - <value>
          table_properties:  # PRIVATE PREVIEW | map[string, string] | Table properties to set on the destination table.
            <key>: <value>
          workday_report_parameters:  # PRIVATE PREVIEW | object | (Optional) Additional custom parameters for Workday Report
            incremental: <bool>  # DEPRECATED | PRIVATE PREVIEW | bool | (Optional) Marks the report as incremental.
            parameters:  # PRIVATE PREVIEW | map[string, string] | Parameters for the Workday report. Each key represents the parameter name (e.g.,
              <key>: <value>
            report_parameters:  # DEPRECATED | PRIVATE PREVIEW | array[...(nested)] | (Optional) Additional custom parameters for Workday Report
              - <value>
      ...
      notifications:  # array[object] | List of notification settings for this pipeline.
        -
          ...
          email_recipients:  # array[string] | A list of email addresses notified when a configured alert is triggered.
            - <value>
      parameters:  # map[string, string] | [Beta] Key/value map of default parameters to use for pipeline execution.
        <key>: <value>
      permissions:  # array[object] | The permissions to apply to this resource.
        -
          ...
          level: CAN_MANAGE  # REQUIRED | enum: CAN_MANAGE, IS_OWNER, CAN_RUN, CAN_VIEW | The permission level to apply. The allowed levels depend on the resource type.
      ...
      restart_window:  # PRIVATE PREVIEW | object | Restart window of this pipeline.
        days_of_week:  # PRIVATE PREVIEW | array[string] | Days of week in which the restart is allowed to happen (within a five-hour windo
          - <value>
        start_hour: <int>  # REQUIRED | PRIVATE PREVIEW | int | An integer between 0 and 23 denoting the start hour for the restart window in th
        time_zone_id: <string>  # PRIVATE PREVIEW | string | Time zone id of restart window. See https://docs.databricks.com/sql/language-man
      root_path: <string>  # string | Root path for this pipeline.
      run_as:  # object | Write-only setting, available only in Create/Update calls. Specifies the user or
      ...
      serverless_compute_id: <string>  # PRIVATE PREVIEW | string | Serverless compute ID specified by the user for serverless pipelines.
