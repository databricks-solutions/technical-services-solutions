...
# MAGIC %md
# MAGIC # SQL Server Reconciliation Wrapper
# MAGIC
...
# MAGIC `get_recon_results()` from `sql_server_recon_main` per row, and appends results to
...
# MAGIC run subsets of tables — labels are whatever you assigned in `table_configs` (often one label
# MAGIC per environment / domain). Multiple labels can be comma-separated.
# MAGIC
# MAGIC **How it works with the other notebooks:**
# MAGIC 1. **sql_server_auto_discover** → MERGEs into `{lakebridge_catalog}.{lakebridge_schema}.{lakebridge_config_table}`
...
# MAGIC 3. **sql_server_recon_main** → imported via `%run`; does the actual reconciliation per table
...
# MAGIC %run "./sql_server_recon_main"

# COMMAND ----------

# Hardcoded: this wrapper is specific to SQL Server. Lakebridge's data_source
