    ...
    async def create_lineage_from_analyzer(
        ...
        try:
            ...
            if lineage_config.parser_type == "sql" and format == "cross_reference":
                ...
                # Fetch helper sheets once for object-subtype classification.
                # Both are optional - missing sheets fall back to legacy
                # TABLE_OR_VIEW typing without crashing.
                async def _safe_get_sheet(name: str):
                    try:
                        return await asyncio.to_thread(analyzer.get_sheet, name)
                    except Exception as exc:
                        log.warning(
                            f"Optional helper sheet '{name}' not available "
                            f"({type(exc).__name__}): subtype refinement may be limited"
                        )
                        return None

                sql_programs_df = await _safe_get_sheet("SQL Programs")
                functions_df = await _safe_get_sheet("Functions")
                ...
                for sheet in sheet_names:
                    ...
                    parsed_data = SQLLineageParser.parse_program_object_xref(
                        df,
                        sql_programs_df=sql_programs_df,
                ...
                # Embedded SQL sheets are best-effort: if present they let the
                # parser refine TABLE_OR_VIEW into TABLE/VIEW/etc.
                sheets_dict = {}
                optional_embedded = [
                    "Embedded SQL Programs",
                    "Embedded SQL Functions",
                    "Embedded SQL Program-Obj Xref",
                    "RAW_PROGRAM_OBJECT_XREF",
                ]
                for name in list(sheet_names) + optional_embedded:
                    try:
                        ...
                        if name in optional_embedded:
                            log.info(
                                f"Optional sheet '{name}' not present "
                                f"(embedded SQL classification will be skipped)"
                            )
                        else:
