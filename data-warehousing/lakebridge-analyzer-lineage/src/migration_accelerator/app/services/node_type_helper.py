...
class NodeTypeHelper:
    ...
    FILE = "FILE"
    FLAT_FILE = "FLAT_FILE"
    TABLE_OR_VIEW = "TABLE_OR_VIEW"  # legacy fallback when subtype is unknown
    TABLE = "TABLE"
    VIEW = "VIEW"
    ...
    # Persisted data-object subtypes (count toward "tables" in summaries / treated like TABLE_OR_VIEW)
    PERSISTED_DATA_TYPES = {
        TABLE_OR_VIEW,
        TABLE,
        VIEW,
        MATERIALIZED_VIEW,
    ...
    # All valid types
    ALL_TYPES = {
        FILE, FLAT_FILE, TABLE_OR_VIEW, TABLE, VIEW, MATERIALIZED_VIEW,
        PROCEDURE, SEQUENCE, INDEX, MACRO,
        MAPPING, SESSION, WORKFLOW, MAPPLET,
    }
    ...
    def is_table_node(cls, node: Dict[str, Any]) -> bool:
        """
        Check if node is a persisted data object (any TABLE/VIEW subtype or flat file).

        Flat files (CSV, TXT, XML, etc.) are treated identically to tables
        for migration ordering, dependency analysis, and insights. All
        TABLE/VIEW subtypes (TABLE, VIEW, MATERIALIZED_VIEW)
        plus the legacy TABLE_OR_VIEW catch-all are accepted.

        Args:
            node: Node dictionary with 'type' field

        Returns:
            True if node is a persisted data object, False otherwise
        """
        t = cls.normalize_type(node.get("type") or "")
        return t in cls.PERSISTED_DATA_TYPES
    ...
    def get_display_name(cls, node_type: str) -> str:
        ...
        Examples:
            ...
            "Table or View"
        """
        return cls.DISPLAY_NAMES.get(cls.normalize_type(node_type), node_type)
    ...
    def normalize_type(cls, node_type: str) -> str:
        ...
        if node_type == "EXTERNAL_TABLE_OR_VIEW":
            return cls.TABLE_OR_VIEW
        return node_type
    ...
    def count_by_type(cls, nodes: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        Returns:
            Dictionary with counts for all node types. The "tables" key sums
            all TABLE/VIEW subtypes (including legacy TABLE_OR_VIEW) so existing
            callers see a stable persisted-object total; per-subtype counts are
            also included for the new view.
        """
        return {
            "files":              sum(1 for n in nodes if n.get("type") == cls.FILE),
            "flat_files":         sum(1 for n in nodes if n.get("type") == cls.FLAT_FILE),
            "tables":             sum(1 for n in nodes if n.get("type") in cls.PERSISTED_DATA_TYPES and n.get("type") != cls.FLAT_FILE),
            "data_targets":       sum(1 for n in nodes if n.get("type") in cls.PERSISTED_DATA_TYPES),
            "table":              sum(1 for n in nodes if n.get("type") == cls.TABLE),
            "view":               sum(1 for n in nodes if n.get("type") == cls.VIEW),
            "materialized_view":  sum(1 for n in nodes if n.get("type") == cls.MATERIALIZED_VIEW),
            "external":           sum(1 for n in nodes if n.get("type") == "EXTERNAL_TABLE_OR_VIEW"),
            "table_or_view":      sum(
                1 for n in nodes if cls.normalize_type(n.get("type") or "") == cls.TABLE_OR_VIEW
            ),
            "procedures":         sum(1 for n in nodes if n.get("type") == cls.PROCEDURE),
            "sequences":          sum(1 for n in nodes if n.get("type") == cls.SEQUENCE),
            "indexes":            sum(1 for n in nodes if n.get("type") == cls.INDEX),
            "macros":             sum(1 for n in nodes if n.get("type") == cls.MACRO),
