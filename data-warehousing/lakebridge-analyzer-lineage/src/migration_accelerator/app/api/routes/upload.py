...
# Uploads are limited to SQL DW and Informatica Lakebridge analyzer outputs only.
ALLOWED_DIALECTS = frozenset({"sql", "informatica"})
_UNSUPPORTED_DIALECT_UPLOAD_MSG = (
    "Only SQL and Informatica (Lakebridge analyzer) exports can be uploaded. "
    f"Allowed dialect values: {', '.join(sorted(ALLOWED_DIALECTS))}."
)
    ...
    try:
        ...
        if dialect not in ALLOWED_DIALECTS:
            ...
            _processing_status[file_id] = {
                ...
                "lineages": [],
                "error": (
                    f"{_UNSUPPORTED_DIALECT_UPLOAD_MSG} "
                    f"(detected or stored dialect: {dialect!r})"
                ),
            }
            log.warning(
                "Rejected upload %s: dialect %r is not accepted for this application",
                file_id,
                dialect,
            )
            return
...
async def upload_file(
    ...
    dialect: Optional[str] = Form(
        None,
        description="sql or informatica only (optional; otherwise auto-detected, must be one of those)",
    ),
    ...
    if dialect and dialect not in ALLOWED_DIALECTS:
        raise HTTPException(
            status_code=400,
            detail=_UNSUPPORTED_DIALECT_UPLOAD_MSG,
        )
    ...
    # Validate the xlsx synchronously - xlsx files are zip archives, so we
    # try to open the bytes as a zip. This rejects empty headers, truncated
    # archives, and bytes that have the PK magic but aren't actually valid
    # zip containers - all of which would otherwise fail asynchronously in
    # the background processor and require status polling to surface.
    import io
    import zipfile

    try:
        with zipfile.ZipFile(io.BytesIO(content)) as zf:
            # An xlsx is a valid zip that contains [Content_Types].xml at the root
            if "[Content_Types].xml" not in zf.namelist():
                raise zipfile.BadZipFile("missing [Content_Types].xml")
    except zipfile.BadZipFile as e:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid xlsx file: {e}",
        )
