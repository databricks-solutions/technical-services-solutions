...
# MAGIC %md
# MAGIC # SQL Server Reconciliation Main
# MAGIC
...
# MAGIC - **Depends on:** `sql_server_transformation_query_generator` (imported via `%run`) — provides
# MAGIC   `get_transformations()` which builds per-column type-aware CASTs so that SQL Server and
# MAGIC   Databricks values compare correctly as strings.
# MAGIC - **Called by:** `sql_server_recon_wrapper` — which iterates over config rows from the metadata
...
# MAGIC %run "./sql_server_transformation_query_generator"

# COMMAND ----------

# MAGIC %run "./sql_server_aggregate_rule_generator"
