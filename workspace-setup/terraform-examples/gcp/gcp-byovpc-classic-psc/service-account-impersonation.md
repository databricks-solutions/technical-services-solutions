...
## Additional Permissions for Backend PSC

Backend Private Service Connect creates PSC endpoints (forwarding rules + internal addresses) and a private DNS zone in the **VPC host project**. Beyond the workspace-creator permissions above, the `privileged-sa` (or the GSA you run Terraform with) also needs these permissions **on the VPC host project**:
...
These are covered by the predefined roles `roles/compute.networkAdmin` and `roles/dns.admin` on the host project, or you can add the individual permissions to the custom `DatabricksAdmin` role above. See the Databricks [customer-managed VPC role requirements](https://docs.databricks.com/gcp/en/security/network/classic/customer-managed-vpc#role-requirements) for the authoritative list.
